...
> it might be better in practice to use a library like Apache Commons Lang, which provides a `EqualsBuilder` and `HashCodeBuilder` 
> or Java 7+'s `Objects.hash` and `Objects.equals` to simplify the process.
